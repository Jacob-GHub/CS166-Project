...
CREATE TABLE
    item (
        ...
        FOREIGN KEY (seller_login, seller_role) REFERENCES users (login, role) ON UPDATE CASCADE ON DELETE RESTRICT
    );
...
CREATE TABLE
    auction (
        ...
        auction_status VARCHAR(20) NOT NULL DEFAULT 'Active' CHECK (auction_status IN ('Active', 'Closed')),
        ...
        FOREIGN KEY (seller_login, seller_role) REFERENCES users (login, role) ON UPDATE CASCADE ON DELETE RESTRICT,
        FOREIGN KEY (winner_login, winner_role) REFERENCES users (login, role) ON UPDATE CASCADE ON DELETE SET NULL
    );
...
CREATE TABLE
    bid (
        ...
        FOREIGN KEY (buyer_login, buyer_role) REFERENCES users (login, role) ON UPDATE CASCADE ON DELETE RESTRICT
    );
...
CREATE TABLE
    payment (
        ...
        FOREIGN KEY (auction_id) REFERENCES auction (auction_id) ON UPDATE CASCADE ON DELETE RESTRICT,
        FOREIGN KEY (buyer_login, buyer_role) REFERENCES users (login, role) ON UPDATE CASCADE ON DELETE RESTRICT
    );
...
CREATE TABLE
    shipment (
        ...
        auction_id INT NOT NULL UNIQUE,
        address VARCHAR(255) NOT NULL,
        shipment_status VARCHAR(20) NOT NULL DEFAULT 'Pending' CHECK (
            shipment_status IN ('Pending', 'Shipped', 'Delivered')
        ),
